 ...
 */
package com.kyant.aura.core.score
...
object Score {
     ...
     * the least suitable is the last. There will be no colors returned if all the input colors were
    ...
    fun score(
        ...
        // total count.
        val colorsHct: MutableList<Hct> = ArrayList(colorsToPopulation.size)
        val huePopulation = IntArray(360)
        var populationSum = 0.0
        colorsToPopulation.entries.forEach { (color, population) ->
            val hct = Hct(color)
            colorsHct.add(hct)
        ...
        return chosenColors.map { hct -> hct.asArgb() }
    }
